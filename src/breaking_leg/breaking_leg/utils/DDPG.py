...
from breaking_leg.utils.HER import HER
from breaking_leg.utils.models_image import actor, critic
...
class DDPG:
    def __init__(self, 
        ...
        self.buffer = HER(maxlen=self.maxlen, batch_size=self.batch_size)
    ...
    def get_action(self, observation, epsilon=0.1):
        ...
        observation = torch.tensor(observation).permute(2, 0, 1).to(self.device).to(torch.float32)
